 ...
 * I decided to not continue with the readback feature because with the display
 * buffer in Ram it is MUCH faster to write to the display and 1k of Ram is
 ...
 * History:
 * v0.6
 * .) moved all RAM(g) read and write to gdisp_lld_KS0108.c
 * .) moved some of the macros back to gdisp_lld_KS0108.c
 * .) added "(void) state" inside a #define in setpin_reset
 *
 * v0.5
 * .) removed KS0108_goto - it is not needed
 * .) moved some #defines to board.h - now ALL hardware relevant defines are
 *    in board.h
 * .) removed (void) state; in setpin_reset
 *
 * v0.4
...
LLDSPEC void gdisp_lld_write_color(GDisplay *g) {
    ...
    if (g->p.color != White){       // set dot
    #if !KS0108_NEED_READ
        *p = (data | bit);
    #endif
        write_data(g, data | bit);
    }
    else{                           //clear dot
      #if !KS0108_NEED_READ
          *p = (data & ~bit);
