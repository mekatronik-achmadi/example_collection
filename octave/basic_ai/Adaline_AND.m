...
%pengenalan fungsi And

ftulis = fopen('JST_AND_ADALINE(MATLAB).txt','w');
disp('Metode Adaline, Pengenalan fungsi AND');
fprintf(ftulis,'Metode Adaline, Pengenalan fungsi AND\n');
disp('Inisialisai : ');
...
for epoch = 1:12
    ...
    for i = 1:4
        ...
        out(i) = clasifi1(net(i));
        if ( out(i) == t(i) )
            bobot(1) = bobot(1);
            bobot(2) = bobot(2);
            bobot(3) = bobot(3);
        else
            bobot(1) = bobot(1) + alpha*(t(i)-net(i))*x1(i);
            bobot(2) = bobot(2) + alpha*(t(i)-net(i))*x2(i);
            bobot(3) = bobot(3) + alpha*(t(i)-net(i));
        end
...
fclose(ftulis);
edit JST_AND_ADALINE(MATLAB).txt;
